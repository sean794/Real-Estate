...
def gu_stats(prices, min_count=3):
    ...
    if len(filtered) < min_count:
        return None
    return {
        "평균가":   round(float(filtered.mean()), 0),
        "중위가":   round(float(filtered.median()), 0),
        "거래수":   int(len(prices)),
        "필터후건수": int(len(filtered)),
    }

def build_summary(trade_dir, rent_dir, out_path):
    summary = {"trade": [], "rent": [], "jeonse_rate": [], "updated": datetime.today().strftime("%Y-%m-%d")}
    ...
    for f in sorted(trade_dir.glob("*.csv")):
        ...
        for gu in SEOUL_GU_CODES.keys():
            ...
            stats = gu_stats(sub)
            if stats:
                per_gu.append({"구": gu, "ym": ym, **stats})
        ...
        for label, lo, hi in SIZE_BINS:
            ...
            gu_list = []
            for gu in SEOUL_GU_CODES.keys():
                sub = size_df[size_df["구"] == gu]["거래금액"]
                stats = gu_stats(sub)
                if stats:
                    gu_list.append({"구": gu, **stats})
            size_gu[label] = sorted(gu_list, key=lambda x: x["평균가"], reverse=True)
        ...
        for label, lo, hi in POPULAR_SIZES:
            ...
            gu_list = []
            for gu in SEOUL_GU_CODES.keys():
                sub = pop_df[pop_df["구"] == gu]["거래금액"]
                stats = gu_stats(sub)
                if stats:
                    gu_list.append({"구": gu, **stats})
            popular_gu[label] = sorted(gu_list, key=lambda x: x["평균가"], reverse=True)
    ...
    for f in sorted(rent_dir.glob("*.csv")):
        ...
        for gu in SEOUL_GU_CODES.keys():
            sub = jeonse_df[jeonse_df["구"] == gu]["보증금"]
            stats = gu_stats(sub)
            if stats:
                per_gu_rent.append({"구": gu, **stats})
        ...
        for label, lo, hi in POPULAR_SIZES:
            ...
            gu_list = []
            for gu in SEOUL_GU_CODES.keys():
                sub = pop_df[pop_df["구"] == gu]["보증금"]
                stats = gu_stats(sub)
                if stats:
                    gu_list.append({"구": gu, **stats})
            popular_rent[label] = sorted(gu_list, key=lambda x: x["평균가"], reverse=True)
